...
model DeviceKPICalculator_numSwiEachDay
  "KPI useful for the analysis of a device"
 ...
 if calc_numSwi
  ...
  Modelica.Blocks.MathInteger.TriggeredAdd triggeredAdd(final use_reset=true,
  ...
  Modelica.Blocks.Sources.SampleTrigger sampleTrigger(period=86400)
    annotation (Placement(transformation(extent={{-16,32},{-4,44}})));
equation
  ...
  connect(sampleTrigger.y, triggeredAdd.reset)
    annotation (Line(points={{-3.4,38},{22,38},{22,58}}, color={255,0,255}));
    annotation(Dialog(enable=use_reaInp),
...
end DeviceKPICalculator_numSwiEachDay;
