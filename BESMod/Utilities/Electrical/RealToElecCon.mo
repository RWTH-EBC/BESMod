...
model RealToElecCon
  ...
  Modelica.Blocks.Interfaces.RealInput PEleLoa[nLoa] "Electrical power"
  ...
  Modelica.Blocks.Math.MultiSum multiSumElec(nu=nLoa)
    annotation (Placement(transformation(extent={{-78,34},{-66,46}})));
equation
  if reverse then
    multiSumElec.y = internalElectricalPin.PElec;
  else
    multiSumElec.y = -1*internalElectricalPin.PElec;
  end if;
  connect(PEleLoa, multiSumElec.u)
