...
model TimeConstantEstimationControlSingleZoneBui
  "Control model to help time constant estimation"
  extends
    Systems.Hydraulical.Control.BaseClasses.PartialThermostaticValveControl(      redeclare replaceable
...
equation
  ...
  connect(const1.y, sigBusDistr.uThrWayVal) annotation (Line(points={{-81,-54},
      ...
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(useProBus.TRoomSet, supTSet.TRoomSet) annotation (Line(
      points={{-119,103},{-162,103},{-162,26}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
      extent={{-3,6},{-3,6}},
      horizontalAlignment=TextAlignment.Right));
end TimeConstantEstimationControlSingleZoneBui;
