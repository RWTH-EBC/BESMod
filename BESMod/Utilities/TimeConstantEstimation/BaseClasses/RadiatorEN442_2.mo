...
model RadiatorEN442_2 "Dynamic radiator for space heating"
  ...
  parameter Real fraRad_nominal(min=0, max=1) = 0.35
    "Fraction radiant heat transfer at nominal conditions";
...
protected
  ...
  parameter Modelica.Units.SI.ThermalConductance UAEle(
    fixed=false,
    min=0,
    start=Q_flow_nominal/((T_a_nominal + T_b_nominal)/2 - ((1 - fraRad_nominal)*
        TAir_nominal + fraRad_nominal*TRad_nominal))/nEle)
  ...
  Modelica.Blocks.Sources.RealExpression QCon[nEle](y={if homotopyInitialization
         then homotopy(actual=(1 - fraRad[i]) * UAEle * (heatPortCon.T - vol[i].T) *
        IBPSA.Utilities.Math.Functions.regNonZeroPower(
        x=(heatPortCon.T - vol[i].T),
        n=n - 1,
        delta=0.05), simplified=(1 - fraRad[i]) * UAEle .* abs(dTCon_nominal[i]) ^ (
        n - 1) * (heatPortCon.T - vol[i].T)) else (1 - fraRad[i]) * UAEle * (heatPortCon.T - vol[i].T) *
        IBPSA.Utilities.Math.Functions.regNonZeroPower(
        x=(heatPortCon.T - vol[i].T),
        n=n - 1,
  ...
  Modelica.Blocks.Sources.RealExpression QRad[nEle](y={if homotopyInitialization
         then homotopy(actual=fraRad[i] * UAEle * (heatPortRad.T - vol[i].T) *
        IBPSA.Utilities.Math.Functions.regNonZeroPower(
        x=(heatPortRad.T - vol[i].T),
        n=n - 1,
        delta=0.05), simplified=fraRad[i] * UAEle * abs(dTRad_nominal[i]) ^ (n - 1)
         * (heatPortRad.T - vol[i].T)) else fraRad[i] * UAEle * (heatPortRad.T - vol[i].T) *
        IBPSA.Utilities.Math.Functions.regNonZeroPower(
        x=(heatPortRad.T - vol[i].T),
        n=n - 1,
...
initial equation
  ...
  for i in 1:nEle loop
    ...
    QEle_flow_nominal[i] = k * UAEle * (fraRad_nominal *
      IBPSA.Utilities.Math.Functions.powerLinearized(
        x=k*TWat_nominal[i] - TRad_nominal,
        n=n,
        x0=0.1*k*(T_b_nominal-TRad_nominal)) + (1-fraRad_nominal) *
      IBPSA.Utilities.Math.Functions.powerLinearized(
