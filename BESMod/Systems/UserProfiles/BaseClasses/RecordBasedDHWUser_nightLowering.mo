...
partial model RecordBasedDHWUser_nightLowering
  ...
  extends
    Systems.UserProfiles.BaseClasses.PartialUserProfiles_nightLowering(
     VolDHWDay=if use_dhwCalc then V_dhwCalc_day else DHWProfile.V_dhw_day,
      mDHW_flow_nominal=DHWProfile.m_flow_nominal);

  replaceable parameter
    Systems.Demand.DHW.RecordsCollection.PartialDHWTap DHWProfile
    annotation (choicesAllMatching=true, Dialog(group="DHW", enable=not
          use_dhwCalc and use_dhw));
