...
model SunWithWeaBus
  "Computes the sun's altitude of the current site using the weather bus information"
  ...
  IBPSA.BoundaryConditions.WeatherData.Bus
      weaBus "Weather data bus" annotation (Placement(transformation(extent={{-122,
            -22},{-80,22}}),     iconTransformation(extent={{-68,92},{-48,112}})));
  Modelica.Blocks.Math.UnitConversions.To_deg toDegLon
    "Longitude from bus in deg"
...
//calculation of the SolarTime (Duffie2006 chap. 1.5)
    ...
    "360 is an angle in degree and 365 is the number of days in one year. The earth rotation in one year is 360 degrees";
      ...
      SolarTime =  StandardTime - TimeCorrection - (DiffWeatherDataTime + Diff_localStandardTime_WeatherDataTime) + 4 * (toDegLon.y)/60 + TimeEquation
...
// declination of sun
  ...
  connect(toDegLon.u, weaBus.lon) annotation (Line(points={{-62,-20},{-101,-20},
