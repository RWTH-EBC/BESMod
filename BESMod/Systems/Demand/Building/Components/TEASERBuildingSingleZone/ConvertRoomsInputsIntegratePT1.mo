...
model ConvertRoomsInputsIntegratePT1
  ...
  Modelica.Blocks.Math.Add3 add3_1(
    k1=-1,
    k2=+1,
    k3=-1) annotation (Placement(transformation(extent={{-20,26},{-8,38}})));
  Modelica.Blocks.Routing.Replicator replicator1(nout=2)
    annotation (Placement(transformation(extent={{74,12},{84,24}})));
  ...
  Modelica.Blocks.Continuous.FirstOrder lossRate(k=1 - useLossRatio,
                                                                 T=
        timeConstLoss)
    annotation (Placement(transformation(extent={{42,40},{58,56}})));
  Modelica.Blocks.Continuous.FirstOrder useRate(k=useLossRatio,T=timeConstUse)
...
equation
  ...
  connect(delayedGains.y, sum1.u)
    annotation (Line(points={{-17,-17.3},{-17,-13.4}}, color={0,0,127}));
  connect(sum1.y, add3_1.u2) annotation (Line(points={{-17,2.7},{-17,22},{-26,22},
          {-26,32},{-21.2,32}}, color={0,0,127}));
  connect(add3_1.y, integrator.u)
    annotation (Line(points={{-7.4,32},{4,32}}, color={0,0,127}));
  connect(replicator1.y[1], add3_1.u3) annotation (Line(points={{84.5,17.85},{88,
          17.85},{88,4},{-6,4},{-6,6},{-21.2,6},{-21.2,27.2}}, color={0,0,127}));
  connect(intGain[1], add2.y) annotation (Line(points={{113,-81},{110,-81},{110,
          -64},{94.6,-64}}, color={0,0,127}));
  connect(replicator1.y[2], add2.u1) annotation (Line(points={{84.5,18.15},{88,18.15},
          {88,-30},{92,-30},{92,-54},{80.8,-54},{80.8,-60.4}}, color={0,0,127}));
  ...
  connect(integrator.y, lossRate.u) annotation (Line(points={{27,32},{34,32},{34,
          48},{40.4,48}}, color={0,0,127}));
  connect(lossRate.y, add3_1.u1) annotation (Line(points={{58.8,48},{66,48},{66,
          68},{-8,68},{-8,44},{-30,44},{-30,36.8},{-21.2,36.8}}, color={0,0,127}));
  connect(integrator.y, useRate.u) annotation (Line(points={{27,32},{34,32},{34,
          22},{30,22},{30,18},{40.4,18}}, color={0,0,127}));
  connect(useRate.y, replicator1.u)
    annotation (Line(points={{58.8,18},{73,18}}, color={0,0,127}));
end ConvertRoomsInputsIntegratePT1;
