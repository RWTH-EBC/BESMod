...
model ConvertRoomsInputsDelayLoss2
  ...
  Modelica.Blocks.Math.Sum sum1(nin=nRooms) annotation (Placement(
        ...
        rotation=90,
        origin={-17,-5})));
  Modelica.Blocks.Math.Gain useRate(k=useLossRatio*10^(-5*(1 - delayFac)))
    annotation (Placement(transformation(extent={{44,8},{64,28}})));
  Modelica.Blocks.Math.Gain lossRate(k=(1 - useLossRatio)*10^(-5*(1 - delayFac)))
...
equation
  ...
  connect(lossRate.y, add3_1.u1) annotation (Line(points={{65,46},{70,46},{70,60},
          {-16,60},{-16,40},{-21.2,40},{-21.2,36.8}}, color={0,0,127}));
  ...
  connect(useRate.y, replicator1.u)
    annotation (Line(points={{65,18},{73,18}}, color={0,0,127}));
...
end ConvertRoomsInputsDelayLoss2;
