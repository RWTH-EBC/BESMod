...
protected
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resIntRoof(
    final G=min(AInt*ratioIntWallAreaTopFloor, ARoof*RoofAreaAtticFactor)*hRad)
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resRoofWin(
    final G=min(ARoof*RoofAreaAtticFactor, ATotWin*ratioWinAreaTopFloor)*hRad)
    if ARoof > 0 and ATotWin > 0 and ratioWinAreaTopFloor > 0 "Resistor between roof and windows"
    annotation (Placement(transformation(
      extent={{-10,-10},{10,10}},
      origin={-154,100})));
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resRoofFloor(
    final G=min(ARoof*RoofAreaAtticFactor, AFloor)*hRad)
    if ARoof > 0 and AFloor > 0 and nFloorLevels < 2 "Resistor between floor plate and roof"
    annotation (Placement(
      transformation(
      extent={{-10,-10},{10,10}},
      rotation=-90,
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resExtWallRoof(
    final G=min(ATotExt*ratioExtWallAreaTopFloor, ARoof*RoofAreaAtticFactor)*hRad)
