...
package BuildingSingleThermalZone "Package for single zone thermal zone models"
  ...
  model BuildingSingleThermalZone "Thermal zone containing moisture balance"
    extends
      BESMod.Systems.Demand.Building.Components.TEASERBuildingSingleZone.BuildingSingleThermalZone.BaseClasses.PartialBuildingSingleThermalZone;
   ...
   parameter Modelica.Units.SI.Temperature TOda_nominal "Nominal outdoor air temperature";
    ...
    replaceable parameter  AixLib.DataBase.Pools.IndoorSwimmingPoolBaseDataDefinition poolParam[nPools]=
       fill(AixLib.DataBase.Pools.IndoorSwimmingPoolDummy(),
                                                      nPools) if use_pools
      "Setup for swimming pools" annotation (Dialog(
        enable=use_pools,
        tab="Moisture",
        group="Pools"));
    replaceable parameter AixLib.DataBase.Walls.WallBaseDataDefinition poolWallParam[nPools] = fill(AixLib.DataBase.Walls.ASHRAE140.DummyDefinition(),
   ...
   if sum(zoneParam.ATransparent) > 0 "Correction factor for solar transmission"
    ...
    replaceable
      ...
      if (sum(zoneParam.AExt) + sum(zoneParam.AWin)) > 0
    ...
    AixLib.Utilities.Sources.HeaterCooler.HeaterCoolerPI heaterCooler(
      ...
      each staOrDyn=not zoneParam.withIdealThresholds) if (ATot > 0 or
      zoneParam.VAir > 0) and (recOrSep and (zoneParam.HeaterOn or zoneParam.CoolerOn))
    ...
    AixLib.ThermalZones.HighOrder.Components.DryAir.VarAirExchange airExc(final V=
          zoneParam.VAir) if (ATot > 0 or zoneParam.VAir > 0) and (
      use_NaturalAirExchange or use_MechanicalAirExchange) and not
      use_moisture_balance "Heat flow due to ventilation"
    ...
    AixLib.BoundaryConditions.InternalGains.Moisture.MoistureGains moistureGains(final
        roomArea=zoneParam.AZone, final specificMoistureProduction=zoneParam.internalGainsMoistureNoPeople)
    ...
    AixLib.ThermalZones.HighOrder.Components.MoistAir.VarMoistAirExchange airExcMoi(final V=
          zoneParam.VAir) if (ATot > 0 or zoneParam.VAir > 0) and (
      use_NaturalAirExchange or use_MechanicalAirExchange) and
      use_moisture_balance "Heat flow due to ventilation"
      annotation (Placement(transformation(extent={{-22,-14},{-6,2}})));
    ...
    AixLib.BoundaryConditions.InternalGains.CO2.CO2Balance cO2Balance(
      ...
      if (ATot > 0 or zoneParam.VAir > 0) and use_C_flow
    ...
    Modelica.Blocks.Interfaces.RealOutput QIntGains_flow[3](each final quantity="HeatFlowRate",
      ...
      // Pools
    AixLib.Fluid.Pools.IndoorSwimmingPool indoorSwimmingPool[nPools](
      poolParam=poolParam,
      poolWallParam=poolWallParam,
      redeclare package WaterMedium = MediumPoolWater,
      each energyDynamics=energyDynamics) if (ATot > 0 or zoneParam.VAir > 0)
       and use_moisture_balance and use_pools
    ...
    AixLib.Fluid.Pools.BaseClasses.AirFlowMoistureToROM airFlowMoistureToROM(
  ...
  protected
   ...
   if sum(zoneParam.AWin) > 0
    ...
    Modelica.Blocks.Sources.RealExpression QIntGainsInternalTot_flow[3](y={-lights.convHeat.Q_flow
      ...
      annotation (Placement(transformation(extent={{94,-46},{98,-34}})));
    AixLib.Utilities.Psychrometrics.MixedHumidity mixedHumidity if (ATot > 0
       or zoneParam.VAir > 0) and use_NaturalAirExchange and
    ...
    AixLib.Utilities.Psychrometrics.X_pTphi x_pTphi if (ATot > 0 or zoneParam.VAir
  ...
  equation
    connect(intGains[2], machinesSenHea.uRel) annotation (Line(points={{80,-100},{
            ...
            127}));
    connect(intGains[3], lights.uRel) annotation (Line(points={{80,-93.3333},{
            80,-86},{50,-86},{50,-68.5},{56,-68.5}},
                                             color={0,0,127}));
  ...
  elseif use_MechanicalAirExchange and not use_NaturalAirExchange then
    ...
    connect(humVolAirROM.y, X_w) annotation (Line(points={{-19.6,-48},{52,-48},{52,
            -8},{86,-8},{86,-76},{96,-76},{96,-70},{110,-70}},
                                         color={0,0,127}));
    connect(cO2Balance.uRel, intGains[1]) annotation (Line(points={{16,-59.6},{
            16,-54},{46,-54},{46,-106.667},{80,-106.667}},         color={0,0,127}));
    ...
    partial model PartialBuildingSingleThermalZone
     ...
     if ATot > 0 or zoneParam.VAir > 0
      ...
      BESMod.Systems.Demand.Building.Components.TEASERBuildingSingleZone.FiveElements ROM(
        ...
        final TotalRExt=zoneParam.TotalRExt,
        final FacRExtRem=zoneParam.FacRExtRem,
        ...
        final TotalRFloor=zoneParam.TotalRFloor,
        final FacRFloorRem=zoneParam.FacRFloorRem,
        ...
        final TotalRRoof=zoneParam.TotalRRoof,
        final FacRRoofRem=zoneParam.FacRRoofRem,
        ...
        final ratioIntWallAreaTopFloor=zoneParam.ratioIntWallAreaTopFloor,
  ...
  <li>March, 2012, by Moritz Lauster:<br/>
    First implementation.
      ...
      Rectangle(
        ...
        fillColor={215,215,215},
        fillPattern=FillPattern.Solid)}));
    end PartialBuildingSingleThermalZone;
  end BaseClasses;
end BuildingSingleThermalZone;
