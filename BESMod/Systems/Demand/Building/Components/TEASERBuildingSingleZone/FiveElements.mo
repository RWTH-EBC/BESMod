...
model FiveElements
  ...
  extends BESMod.Systems.Demand.Building.Components.TEASERBuildingSingleZone.FourElements(AArray=cat(
        1,
...
protected
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resIzeIze[sum({i
    for i in 1:(nIze - 1)})](final G=AixLib.ThermalZones.ReducedOrder.RC.BaseClasses.GSurfSurf(
                                                           AIze, hRad))
