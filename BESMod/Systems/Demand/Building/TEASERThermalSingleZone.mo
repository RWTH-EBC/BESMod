...
model TEASERThermalSingleZone
  ...
  parameter Boolean useUserProfileNatVent = false
    "=true to take the air exchange rate of the zone from the user profile instead of letting AixLib's VentilationController derive it from the zone record. The profile then is the whole air exchange: the controller, and with it the record's baseACH, maxUserACH, maxOverheatingACH, maxSummerACH and winterReduction, is switched off so that the two are not added up";
  ...
  BESMod.Systems.Demand.Building.Components.TEASERBuildingSingleZone.BuildingSingleThermalZone.BuildingSingleThermalZone thermalZone[nZones](
    ...
    each use_NaturalAirExchange=not useUserProfileNatVent,
