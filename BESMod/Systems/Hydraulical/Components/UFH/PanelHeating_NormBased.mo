...
model PanelHeating_NormBased
  "A panel heating for e.g. floor heating with discretization based on DIN EN 1264 by fst"
  ...
  parameter Modelica.Units.SI.Length Spacing=Modelica.Constants.pi*
      k_top_eff*floorHeatingType.diameter*
  ...
  // Equations 7.91 (for heat flow up) and 7.93 (for heat flow down) from page 41
  final parameter Modelica.Units.SI.Temperature T_Floor_nom= if isFloor then
  ...
  final parameter Modelica.Units.SI.CoefficientOfHeatTransfer k_top_eff=
  floorHeatingType.q_dot_nom/AixLib.Fluid.HeatExchangers.ActiveWalls.BaseClasses.logDT({
        floorHeatingType.Temp_nom[1],floorHeatingType.Temp_nom[2],T_Floor_nom})
        "Effective heat transfer coefficient based on EN 1264-2";
  ...
  BESMod.Systems.Hydraulical.Components.UFH.PanelHeatingSegment panelHeatingSegment[dis](
    redeclare package Medium = Medium,
    each final A=A/dis,
    each final eps=eps,
    each final T0=T0,
    each final VWater=VWater/dis,
    each final kTop=k_top_eff,
