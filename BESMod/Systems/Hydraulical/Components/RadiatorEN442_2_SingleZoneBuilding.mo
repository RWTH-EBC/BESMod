...
model RadiatorEN442_2_SingleZoneBuilding
  "Dynamic radiator for space heating of a single zone builing model with variable UA value"
  ...
  Modelica.Blocks.Interfaces.RealInput TDryBulNorm
    "Outdoor temperatur normiert with nominal and used for correction of UA value"
    annotation (Placement(transformation(extent={{-126,66},{-86,106}})));
...
protected
  ...
  Modelica.Units.SI.ThermalConductance UAEle(
    fixed=false,
    min=0,
...
equation
  ...
  UAEle = (Q_flow_nominal/((T_a_nominal + T_b_nominal)/2 - ((1 - fraRad)*
        TAir_nominal + fraRad*TRad_nominal))/nEle) * (TZoneSet/TAir_nominal*UAFacTZoneSet + 1/TDryBulNorm*UAFacTDryBul + UAFacConst);
...
end RadiatorEN442_2_SingleZoneBuilding;
