...
model FMIReplaceableDistribution
  ...
  parameter Boolean use_p_in_Gen=true
    "= true to use a pressure from connector, false to output Medium.p_default"
    annotation(Evaluate=true);
  parameter Boolean use_p_in_Bui=true
    "= true to use a pressure from connector, false to output Medium.p_default"
    annotation(Evaluate=true);
  ...
  Modelica.Blocks.Sources.RealExpression dpDisGen[distribution.nParallelSup](y=distribution.portGen_in.p -
        distribution.portGen_out.p) if use_p_in_Gen
  ...
  IBPSA.Fluid.FMI.Adaptors.Inlet bouInlBui[distribution.nParallelSup](
    ...
    each final allowFlowReversal=allowFlowReversal,
    each final use_p_in=use_p_in_Bui)
  ...
  Modelica.Blocks.Sources.RealExpression dpDisBui[distribution.nParallelSup](y=
        distribution.portBui_in.p - distribution.portBui_out.p) if use_p_in_Bui
    "Pressure drop of the component"
  ...
  IBPSA.Fluid.FMI.Interfaces.Outlet portBui_out[distribution.nParallelSup](
    redeclare each final package Medium = Medium,
    each final allowFlowReversal=allowFlowReversal,
    each final use_p_in=use_p_in_Bui) "Outlet for the distribution to the building"
  ...
  Modelica.Blocks.Sources.RealExpression dpDisDHW(y=
       distribution.portDHW_in.p - distribution.portDHW_out.p) if use_p_in_DHW and use_dhw
    "Pressure drop of the component"
  ...
  Modelica.Blocks.Math.Feedback pOutGen[distribution.nParallelSup] if use_p_in_Gen
    ...
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=270,
        origin={-50,12})));
  Modelica.Blocks.Math.Feedback pOutBui[distribution.nParallelSup] if use_p_in_Bui
...
public
  replaceable BESMod.Systems.Hydraulical.Distribution.BaseClasses.PartialDistribution distribution
    constrainedby BaseClasses.PartialDistribution(
    use_inFMU=true,
...
equation
  ...
  if not use_dhw then
    connect(distribution.portDHW_out, distribution.portDHW_in)
                                                              annotation (Line(
      points={{18,-3.6},{18,-10.8}},
      color={0,127,255},
      pattern=LinePattern.Dash));
  end if;
    connect(portGen_in, bouInlGen.inlet)
    annotation (Line(points={{-110,40},{-61,40}}, color={0,0,255}));
  connect(bouInlGen.port_b, distribution.portGen_in) annotation (Line(points={{-40,
          40},{-24,40},{-24,14.4},{-18,14.4}}, color={0,127,255}));
  connect(bouOutGen.outlet, portGen_out)
    annotation (Line(points={{-61,-20},{-110,-20}}, color={0,0,255}));
  connect(bouOutGen.port_a, distribution.portGen_out) annotation (Line(points={{
          -40,-20},{-24,-20},{-24,7.2},{-18,7.2}}, color={0,127,255}));
  connect(distribution.portBui_out, bouOutBui.port_a) annotation (Line(points={{18,14.4},
          {26,14.4},{26,80},{40,80}},          color={0,127,255}));
  connect(bouOutBui.outlet, portBui_out)
    annotation (Line(points={{61,80},{110,80}}, color={0,0,255}));
  connect(distribution.portBui_in, bouInlBui.port_b) annotation (Line(points={{18,
          7.2},{18,6},{30,6},{30,26},{40,26}}, color={0,127,255}));
  connect(bouInlBui.inlet, portBui_in)
    annotation (Line(points={{61,26},{110,26}}, color={0,0,255}));
  connect(distribution.portDHW_out, bouOutDHW.port_a) annotation (Line(points={{
          18,-3.6},{18,-2},{34,-2},{34,-26},{40,-26}}, color={0,127,255}));
  connect(distribution.portDHW_in, bouInlDHW.port_b) annotation (Line(points={{18,
          -10.8},{28,-10.8},{28,-80},{40,-80}}, color={0,127,255}));
