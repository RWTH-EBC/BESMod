...
partial model PartialDistributionStorageSerie "Partial model to later extent"
 ...
 replaceable parameter
  ...
  AixLib.Fluid.Storage.BufferStorage storageBuf(
    ...
    final m2_flow_nominal=mSup_flow_nominal[1],
...
equation
  ...
  connect(hea.u, sigBusDistr.uHRAftBuf) annotation (Line(points={{38,96},{32,96},
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(senT_heatingFlow.port_b, hea.port_a) annotation (Line(points={{28,76},
          {34,76},{34,90},{40,90}}, color={0,127,255}));
