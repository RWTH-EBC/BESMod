...
partial model PartialDistributionNoStorageForHeating
 ...
 replaceable parameter
  ...
  Modelica.Blocks.Sources.RealExpression reaExpTStoBufTop(final y(
      unit="K",
      displayUnit="degC") = senT_heatingFlow.T)
    annotation (Placement(transformation(
        extent={{-10,-5},{10,5}},
        rotation=0,
        origin={-30,87})));
  Modelica.Blocks.Sources.RealExpression reaExpTStoBufBot(final y(
      unit="K",
      displayUnit="degC") = senT_heatingReturn.T)  annotation (Placement(
        transformation(
        extent={{-10,-5},{10,5}},
        rotation=0,
        origin={-30,79})));
 ...
 if dhwParameters.use_hr
  ...
  IBPSA.Fluid.Sensors.TemperatureTwoPort senT_heatingFlow(
    redeclare final package Medium = Medium,
    final allowFlowReversal=allowFlowReversal,
    m_flow_nominal=m_flow_nominal[1],
    tau=temperatureSensorData.tau,
    ...
    final transferHeat=temperatureSensorData.transferHeat,
    TAmb=temperatureSensorData.TAmb,
    tauHeaTra=temperatureSensorData.tauHeaTra) annotation (Placement(
        transformation(
        extent={{7,8},{-7,-8}},
        rotation=180,
        origin={15,64})));
  IBPSA.Fluid.Sensors.TemperatureTwoPort senT_heatingReturn(
    redeclare final package Medium = Medium,
    final allowFlowReversal=allowFlowReversal,
    m_flow_nominal=m_flow_nominal[1],
    tau=temperatureSensorData.tau,
...
equation
  ...
  connect(hea.u, sigBusDistr.uHRAftBuf) annotation (Line(points={{38,96},{32,96},
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(hea.port_a, senT_heatingFlow.port_b) annotation (Line(points={{40,90},
          {30,90},{30,64},{22,64}}, color={0,127,255}));
  connect(pasThrHeaRodBuf.port_a, senT_heatingFlow.port_b) annotation (Line(
        points={{40,64},{22,64}},                 color={0,127,255}));
  connect(portBui_in[1], senT_heatingReturn.port_a)
    annotation (Line(points={{100,40},{54,40},{54,34},{4,34}},
                                                      color={0,127,255}));
  connect(senT_heatingReturn.port_b, threeWayValveWithFlowReturn.portBui_a)
    annotation (Line(points={{-10,34},{-44,34},{-44,68},{-60,68}}, color={0,127,
          255}));
  connect(threeWayValveWithFlowReturn.portBui_b, senT_heatingFlow.port_a)
    annotation (Line(points={{-60,72},{-50,72},{-50,58},{0,58},{0,64},{8,64}},
        color={0,127,255}));
