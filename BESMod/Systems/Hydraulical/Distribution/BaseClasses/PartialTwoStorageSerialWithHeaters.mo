...
partial model PartialTwoStorageSerialWithHeaters
  "Partial two storage model with heaters"
  extends PartialTwoStorageSerial(
...
equation
  ...
  connect(eneKPICalAftBufBoi.KPI, outBusDist.PBoiAftBuf) annotation (Line(points={{17.8,
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(hea.Pel, multiSum.u[4]) annotation (Line(points={{61,96},{76,96},{76,102},
          {-126,102},{-126,-111},{-58,-111}}, color={0,0,127}));
  connect(resValToBufSto.port_b, boi.port_a) annotation (Line(points={{0,160},{2,
          160},{2,124},{32,124},{32,120},{40,120}}, color={0,127,255}));
  connect(resValToBufSto.port_b, hea.port_a) annotation (Line(points={{0,160},{2,
          160},{2,124},{32,124},{32,98},{30,98},{30,90},{40,90}}, color={0,127,255}));
  connect(resValToBufSto.port_b, pasThrEleHeaBuf.port_a) annotation (Line(
        points={{0,160},{2,160},{2,124},{32,124},{32,98},{30,98},{30,90},{34,90},
          {34,64},{40,64}}, color={0,127,255}));
end PartialTwoStorageSerialWithHeaters;
