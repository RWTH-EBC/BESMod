...
record BufferStorageBaseDataDefinition
  ...
  parameter Modelica.SIunits.Power QHR_flow_nominal=0 annotation (Dialog(group="Heating Rod"));
  ...
  parameter Modelica.SIunits.TemperatureDifference dTLoadingHC2  "Temperature difference for loading of first heating coil" annotation (Dialog(group="Loading", enable=use_HC2));
  ...
  parameter Modelica.SIunits.HeatFlowRate QHC2_flow_nominal=Q_flow_nominal  "Nominal heat flow rate in first heating coil" annotation (Dialog(group="Loading", enable=use_HC2));
  parameter Modelica.SIunits.Length lengthHC2=floor((h * fHeiHC2 / pipeHC2.d_o)) * pipeHC2.d_o / sin(atan(pipeHC2.d_o/(d*fDiaHC2))) "Lenght of first HC" annotation (Dialog(tab="Calculated", group="Loading", enable=use_HC2));
  parameter Modelica.SIunits.MassFlowRate mHC2_flow_nominal "Nominal mass flow rate of HC fluid" annotation (Dialog(group="Loading", enable=use_HC2));
  ...
  parameter Modelica.SIunits.CoefficientOfHeatTransfer hConHC2=(2/pipeHC2.d_i+2/pipeHC2.d_o)/(((max(dTLoadingHC2, dTLoaMin)*2*Modelica.Constants.pi*lengthHC2)/QHC2_flow_nominal)-(1/pipeHC2.lambda * log(pipeHC2.d_o/pipeHC2.d_i)))
                                                                                                                                                                                                        "Model assumptions Coefficient of Heat Transfer HC2 <-> Heating Water"
                                                                                                                                                                                                        annotation (Dialog(tab="Calculated", group="Loading", enable=use_HC2));
  parameter Modelica.SIunits.Velocity vHC2_nominal = mHC2_flow_nominal / (rho * (pipeHC2.d_i * Modelica.Constants.pi / 2) ^ 2)  "Fluid velocity in pipe of HC 1 at nominal conditions"
                                                                                                                                                                                      annotation (Dialog(tab="Calculated", group="Loading", enable=use_HC2));
