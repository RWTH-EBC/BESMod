...
model SingleZoneBuildingAndDHWControl
  ...
  parameter Integer nRooms "Number of real rooms with a set Terperatrue";
  ...
  BuildingSupplyTemperatureSetpoints.IdealHeatingCurveHOMtoROM
                               TSetBuiSup(
    nZones=nZones,
    nRooms=nRooms,
    TSup_nominal=TSup_nominal,
  ...
  Modelica.Blocks.Interfaces.RealInput TRoomSet[nRooms](each final unit="K",
      each final displayUnit="degC") "Zones set temperatures"
    annotation (Placement(transformation(extent={{-240,-164},{-200,-124}})));
equation
  ...
  connect(supCtrThrWayVal.y, sigBusDistr.uThrWayVal) annotation (Line(points={{58,130},
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(TRoomSet, TSetBuiSup.TRoomSet) annotation (Line(points={{-220,-144},{
          -190,-144},{-190,-100},{-172,-100}}, color={0,0,127}));
...
end SingleZoneBuildingAndDHWControl;
