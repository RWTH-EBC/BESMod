...
model UFHTransferSystemPressureBased
  extends
    BESMod.Systems.Hydraulical.Transfer.BaseClasses.PartialWithPipingLosses(
      dp_design=dpPipSca_design .+ dpUFH_design, nHeaTra=1.1);
  ...
  BESMod.Systems.Hydraulical.Components.UFH.PanelHeating ufh[nParallelDem](
    ...
    Spacing=0.2,
    each final dis=dis,
    final A=UFHParameters.area/dis,
    each final T0=T_start,
    each calcMethod=AixLib.ThermalZones.HighOrder.Components.Types.CalcMethodConvectiveHeatTransferInsideSurface.Custom_hCon,
    hCon_const=7,
    cTop=1100000,
    cDown=10000,
...
equation
  ...
  connect(senTRet.y, outBusTra.TRet) annotation (Line(points={{-49,-74},{-50,
      ...
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(res.port_b, val.port_a) annotation (Line(points={{-20,40},{-14,40},{
          -14,54},{2,54},{2,47}}, color={0,127,255}));
  connect(val.port_b, ufh.port_a) annotation (Line(points={{2,27},{4,27},{4,20},
          {8.33333,20}}, color={0,127,255}));
  connect(traControlBus.opening, val.y) annotation (Line(
      points={{0,100},{0,86},{12,86},{12,50},{20,50},{20,37},{15.2,37}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
...
end UFHTransferSystemPressureBased;
