...
partial record TransferDataBaseDefinition "Data record for hydraulic heat transfer system"
  ...
  parameter Integer nZones "Numer of zones heated"
          ...
          "Building"));
  parameter Modelica.SIunits.HeatFlowRate Q_flow_nominal[nZones] "Nominal heat flow rate"
                                                                                         annotation(Dialog(group=
          "Building"));
  parameter Modelica.SIunits.Area AFloor "Net floor area of one floor"        annotation(Dialog(group=
          "Building"));
  parameter Modelica.SIunits.Length heiBui "Building height" annotation(Dialog(group=
          "Building"));
  ...
  parameter BESMod.Systems.Hydraulical.Transfer.Types.HeatTransferSystemType traType "Heat transfer system type"  annotation(Dialog(group=
          "Volume"));
  parameter Modelica.SIunits.Volume vol = BESMod.Systems.Hydraulical.Transfer.Functions.GetAverageVolumeOfWater(sum(Q_flow_nominal),traType)  "Volume of water in whole heat distribution and transfer system" annotation(Dialog(group=
          "Volume"));
  // Pressure
  ...
  parameter Real zf(min=1.0, max=10.0, unit="1") = BESMod.Systems.Hydraulical.Transfer.Functions.GetSurchargeFactorForHydraulicResistances(typeOfHydRes)
      ...
      annotation(Dialog(group="Pressure"));
  parameter Modelica.SIunits.PressureDifference dpHeaDistr_nominal=pressureDropPerLen*zf*2*(2 * sqrt(AFloor) + heiBui) "Pressure difference of heat distribuition system including piping plus pressure resistances but excluding UFH piping / heating circuit distributor. Actually L * W * H (factor 2 for flow and return)."
      annotation(Dialog(group="Pressure"));
  ...
  parameter Modelica.SIunits.PressureDifference dpRad_nominal[nZones]=fill(perPreLosRad*dpHeaDistr_nominal, nZones)
    "Pressure drop at nominal mass flow rate in radiator"  annotation(Dialog(group="Radiator"));
  // Valves
  ...
  parameter Modelica.SIunits.PressureDifference dpHeaSysValve_nominal[nZones]= (dpRad_nominal .+ dpHeaSysPreValve_nominal) ./ (1 .- valveAutho) "Nominal pressure drop over valve when fully opened at m_flowValve_nominal" annotation (Dialog(group="Thermostatic Valve"));
  ...
  parameter Modelica.SIunits.PressureDifference dpHeaSysPreValve_nominal[nZones]=if use_hydrBalAutom then max(dpRad_nominal) .- (dpRad_nominal) else fill(0, nZones) "Pressure difference of each branch in heat distribution system as pre set value for valves (hydraulic balance)"
