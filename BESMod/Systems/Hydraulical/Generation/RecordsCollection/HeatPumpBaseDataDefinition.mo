...
partial record HeatPumpBaseDataDefinition
  ...
  parameter Modelica.SIunits.HeatFlowRate QGen_flow_nominal "Nominal heating load at outdoor air temperature" annotation(Dialog(group="Design"));

  // Temperature Levels
  parameter Modelica.SIunits.Temperature TOda_nominal "Nominal outdoor air temperature" annotation(Dialog(group="Design"));
  parameter Modelica.SIunits.Temperature THeaTresh "Heating treshhold" annotation(Dialog(group="Design"));
  parameter
    BESMod.Systems.Hydraulical.Generation.Types.GenerationDesign
    genDesTyp "Type of generation system design" annotation (Dialog(
      group="Design"));
  parameter Modelica.SIunits.HeatFlowRate QPri_flow_nominal=if genDesTyp ==
  ...
  parameter Modelica.SIunits.HeatFlowRate QSec_flow_nominal=if genDesTyp ==
    ...
    annotation (Dialog(
      group="Design"));
  parameter Modelica.SIunits.Temperature TBiv=TOda_nominal
  ...
  parameter Modelica.SIunits.HeatFlowRate QGenBiv_flow_nominal = QGen_flow_nominal *(TBiv - THeaTresh)  / (TOda_nominal - THeaTresh) "Nominal heat flow rate at bivalence temperature" annotation (
     Dialog(group="Design"));

  // Generation: Heat Pump
  parameter Real scalingFactor=1 "Scaling-factor of vapour compression machine";
  parameter Modelica.SIunits.MassFlowRate mEva_flow_nominal=1
  ...
  parameter Modelica.SIunits.Frequency refIneFre_constant=0
