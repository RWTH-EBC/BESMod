...
partial record ThreeWayValve
  ...
  parameter Modelica.SIunits.PressureDifference dp_nominal[2](displayUnit="Pa")
    "Nominal pressure drop of connected resistances without the valve";

  parameter Modelica.SIunits.PressureDifference dpValve_nominal=max(dp_nominal) / (1 - valveAutho)
    "Nominal pressure drop of fully open valve, used if CvData=IBPSA.Fluid.Types.CvTypes.OpPoint";
  parameter Modelica.SIunits.PressureDifference dpFixed_nominal[2]=max(dp_nominal) .- (dp_nominal)
